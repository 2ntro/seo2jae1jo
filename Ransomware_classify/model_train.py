...
os.chdir('C:/sharedfolder/trainmodel')#작업할 디렉토리 설정

#가장먼저 추출한 문자열 들을 가지고 옴
with open('../software/behavior_api_order.csv', 'r', encoding='utf-8') as f:
    lines = csv.reader(f)
    software_sentences = [line for line in lines]
with open('../behavior_api_order.csv', 'r', encoding='utf-8') as f:
    lines = csv.reader(f)
    malware_sentences = [line for line in lines]
#len(software_sentences)
#print(len(malware_sentences))
...
#features = set(malware_model.wv.index_to_key[-50:]) - set(software_model.wv.index_to_key[-50:])
#features = set(software_model.wv.index_to_key[-50:]) - set(malware_model.wv.index_to_key[-50:])
#features = set(software_model.wv.index_to_key[:100])&set(malware_model.wv.index_to_key[:100])#교집합.
#features = malware_model.wv.index_to_key[:100] #그냥 평범한 모델 API 빈도 순서
...
vector_size = 10 #벡터 크기
min_count=0# 최소 단어 숫자
window = 5#컨텍스트 자를 크기 10
dm =1# 학습 방식. 문장 => 단어  / 단어 문장
addition_string = "_" + "malware"# 추가로 적고 싶은 말
...
def bayesOpt(train, features):#가장먼저 호출됨.
    ...
    best_target = max(bo.res, key=lambda x: x['target'])['target']
    print(best_target)
    print(best_params)
...
#이런식으로 호출 하면됨.
    ...
    #booster.save_model(doc2vec_model_name+'.xgboost')
...
malware_labels.shape
    ...
    #데이터 셋 합치기.
arrays = np.vstack(malware_arrays)
labels = np.hstack(malware_labels)
bayesOpt(arrays,labels)'''